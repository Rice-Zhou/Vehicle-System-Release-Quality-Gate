...
- Therefore, regardless of Issue count, the Snapshot Repository executes a fixed 5 reads (Release, Header, Issues, Paths, Gaps), with header/issues/path/gap each read exactly once. The real `JdbcProjectAuthorizer` executes 1 additional membership SQL, bringing the complete authorized read path to 6 database round trips. Both counts are constant with respect to Issue count, so there is no per-Issue query. The current 20-Issue query-shape test isolates the Repository with a fake authorizer and strictly proves 5 Repository calls; it is not reported as the total database-query count.
...
`TraceabilityReplayTest` first completes a Snapshot, then uses an authenticated JWT with `SCOPE_traceability:read` to call an exact `snapshotId` GET through the real Security filter, Controller, Task 1 public DTO, Application, `JdbcProjectAuthorizer`, and Repository. It saves `response.contentAsByteArray` and `snapshot.contentDigest` from the public JSON. The test then inserts an M2.4 `revision+1` (INVALID) and a new Issue Snapshot, calls the same public GET again, and requires byte-for-byte identical response bytes and an unchanged digest. It also fixes the top-level public JSON field order, canonical Issue order, and canonical four-edge Path order. The read path has no current/latest authority query, so future facts can affect only a new Run/Snapshot and cannot rewrite a historical response.
...
## Fix round 1 — Public Replay and query-count correction

- Independent review result: 0 Critical, 1 Important, and 1 Minor. The Important finding identified that the initial Replay directly serialized the Application `TraceabilitySnapshotResult`, so it could not prove the historical stability of the Task 1 public DTO, Controller, and real GET response bytes. The Minor finding identified that the initial report described 5 Repository calls under a fake authorizer as the complete database-query count.
- RED: before changing the test, the public-boundary structural check failed explicitly with `RED: replay does not capture bytes from the public GET boundary`; the old source also did not contain `SCOPE_traceability:read`. This directly proved the reviewed coverage gap rather than a production-behavior failure.
- GREEN: Replay now uses `@AutoConfigureMockMvc` and issues an exact `snapshotId` GET with the fixture Project member's issuer/subject and `SCOPE_traceability:read`. The request actually traverses Security, Controller, the Task 1 public DTO, Application, `JdbcProjectAuthorizer`, and JDBC; it no longer calls `writeValueAsBytes(applicationResult)`. `compileTestKotlin --rerun-tasks` returned `BUILD SUCCESSFUL`, and the structural check also proves that the public GET/read scope is present and direct Application serialization is absent.
- Public JSON determinism: the test requires the top-level fields to be exactly `snapshot,issues`, two Issues in Task 1 canonical order, and the first Issue Path to be exactly `ISSUE_COMMIT,COMMIT_BUILD,BUILD_ARTIFACT,ARTIFACT_RELEASE`. It parses and compares `snapshot.contentDigest` from the public JSON before and after the mutation and compares both `response.contentAsByteArray` values byte for byte. Any Controller mapping, Task 1 DTO field/order, or serialization regression breaks the real PostgreSQL Replay case.
- Query-count ruling: the database-free 20-Issue shape test proves only 5 Snapshot Repository calls. A production complete Snapshot GET also includes 1 membership SQL from `JdbcProjectAuthorizer`, for a total of 6 database round trips. Both remain constant with Issue count, so the no-N+1 conclusion is unchanged. No datasource proxy or brittle instrumentation was introduced solely for exact counting.
- The final non-PostgreSQL regression remains 7 suites, 25 tests, 0 failure/error/skip; Contract remains 34 operations PASS, and the acceptance validator remains 37/37 PASS.
- The local Docker limitation is unchanged: rerunning the focused `TraceabilityReplayTest` failed 1/1 while initializing `DockerClientProviderStrategy`; production/test compilation succeeded, but the database fixture and public response bytes/contentDigest assertions were never reached. Exact-head Linux/Docker CI for the fix commit must provide GREEN Evidence.

## CI fix round 1 — Issue Snapshot fixture version authority

- The first exact-head CI runs had the same single failure on both sides: Chinese Run `33922684732` / Job `101184297620` / Artifact `9955777800`, and English Run `33922684588` / Job `101184297309` / Artifact `9955765741`. `TraceabilityReplayTest.later edge revision...` triggered `DuplicateKeyException` / `uq_issue_snapshot_release_version` while appending a new Issue Snapshot; the production query and Replay comparison assertions had not failed.
- Root cause: `seed(issueCount=2)` first used `appendSnapshotIssues` to create version 2 for the same Release, while `appendLatestSnapshot` still hard-coded version 2. `appendUnsupportedLatestSnapshot` contained the same hard-coded value; although the existing default-seed path did not collide, it had the same structural defect for a fixture that already contained version 2.
- RED: the first exact-head CI runs on both sides are the real PostgreSQL RED. The Replay regression now also uses an independent SQL query to require existing versions to be exactly `1,2` and versions after the append to be exactly `1,2,3`; retaining hard-coded 2 fails at the unique constraint before that assertion and cannot be bypassed by changing the test expectation.
- Fix: only the shared test fixture changed. `insertSnapshot` no longer accepts a caller-provided version and now uses parameterized SQL in the current transaction to query `COALESCE(MAX(snapshot_version), 0) + 1` by `(project_id, release_id)`. `appendSnapshotIssues`, `appendLatestSnapshot`, and `appendUnsupportedLatestSnapshot` all reuse this helper, removing parallel hard-coded values; default seed remains v1→v2, while the multi-Issue seed becomes v1→v2→v3.
- Local evidence: all production and test sources compiled successfully with `compileTestKotlin --rerun-tasks`. The local Docker limitation still prevents a real PostgreSQL GREEN, so consecutive versions and the complete public bytes/digest Replay must be proved by Linux/Docker CI bound to the exact CI-fix commit; compilation is not reported as a semantic PASS.
- Scope: no production Schema, Migration, Repository, Application, Controller, or DTO changed; no second version authority, fallback, or test-only production hook was added.
