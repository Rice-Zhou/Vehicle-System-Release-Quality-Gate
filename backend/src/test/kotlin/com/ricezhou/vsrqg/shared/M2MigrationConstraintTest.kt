...
class M2MigrationConstraintTest : PostgresIntegrationTest() {
    ...
    fun `flyway creates the complete M2 authority schema and read only manifest edge view`() {
        ...
        assertThat(tables).containsExactlyElementsOf(m2Tables.sorted())
    ...
    fun `M2 schema declares all required foreign keys uniqueness checks and indexes`() {
        ...
        assertConstraintNames(
            ...
            setOf(
                ...
                "fk_snapshot_edge_snapshot_project", "fk_snapshot_gap_snapshot_project", "fk_snapshot_gap_issue_project",
                "fk_snapshot_gap_release_project",
            ),
        )
        assertConstraintNames(
            "u",
            setOf(
                ...
                "uq_trace_snapshot_digest", "uq_snapshot_edge_digest", "uq_snapshot_gap_digest",
            ),
        )
        assertConstraintNames(
            "c",
            setOf(
                ...
                "ck_snapshot_edge_confidence", "ck_snapshot_edge_status", "ck_snapshot_gap_digest",
            ),
        )
        ...
        val requiredTriggers = setOf(
            ...
            "immutable_traceability_snapshot_gap",
        )
    ...
    private fun insertIssueCommitRevision(id: String, edgeId: String, revision: Int, previousId: String?, previousRevision: Int?, issueId: String, commitId: String, sourceType: String, sourceReference: String) {
        jdbc.sql(
            """
            ...
            """.trimIndent(),
        ).param("id", id).param("projectId", "project_" + issueId.removePrefix("issue_").removeSuffix("_2"))
    ...
    private fun insertCommitBuildRevision(id: String, edgeId: String, revision: Int, previousId: String?, previousRevision: Int?, commitId: String, buildId: String, sourceType: String, sourceReference: String) {
        jdbc.sql(
            """
            INSERT INTO commit_build_edge_revision(
              ...
              confidence, verification_status, validator_version, previous_revision_id, previous_revision,
              content_digest, created_at
            ) VALUES (:id, 'project_revision', :edgeId, :revision, :commitId, :buildId, :sourceType, :sourceReference,
                      'HIGH', 'VALID', 'validator-v1', :previousId, :previousRevision, :digest, now())
            """.trimIndent(),
        ).param("id", id).param("edgeId", edgeId).param("revision", revision).param("commitId", commitId)
    ...
    private fun insertBuildArtifactRevision(id: String, edgeId: String, revision: Int, previousId: String?, previousRevision: Int?, buildId: String, artifactId: String, sourceType: String, sourceReference: String) {
        jdbc.sql(
            """
            INSERT INTO build_artifact_edge_revision(
              ...
              confidence, verification_status, validator_version, previous_revision_id, previous_revision,
              content_digest, created_at
            ) VALUES (:id, 'project_revision', :edgeId, :revision, :buildId, :artifactId, :sourceType, :sourceReference,
                      'HIGH', 'VALID', 'validator-v1', :previousId, :previousRevision, :digest, now())
            """.trimIndent(),
        ).param("id", id).param("edgeId", edgeId).param("revision", revision).param("buildId", buildId)
