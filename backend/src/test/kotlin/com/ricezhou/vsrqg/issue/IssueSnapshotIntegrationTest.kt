...
class IssueSnapshotUseCaseTest {
    ...
    fun `authorization completes before idempotency or authority locks`() {
        val fixture = Fixture(denyAuthorization = true)
    ...
    private class Fixture(
        ...
        denyAuthorization: Boolean = false,
    ) {
        ...
        private val authorizer = ProjectAuthorizer { _, _, permission ->
            ...
            if (denyAuthorization) throw AccessDeniedException("denied")
    ...
    private class FakeRepository(now: Instant) : IssueSnapshotRepository {
        ...
        override fun findContext(releaseId: String, sourceId: String) = context
        ...
        jdbc.sql(
            """INSERT INTO normalized_issue(id, project_id, source_id, source_issue_id, title,
               ...
               VALUES (:id, :projectId, :sourceId, :key, :key, 'HIGH', 'OPEN', 'open', 'v1',
                 :reference, now(), 'mapping-v1', false, :digest, 'normalized-issue-facts/v1', now())""",
        ).param("id", issueId).param("projectId", projectId).param("sourceId", sourceId).param("key", key)
            .param("reference", "fixture:$key").param("digest", hash("fact-$key")).update()
        jdbc.sql(
