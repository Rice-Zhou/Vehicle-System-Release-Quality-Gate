...
No new build, API, ADB action or database migration ran in this round, and none of these behaviors is claimed implemented. Submission checks cover documents, baseline contracts, links, bilingual Pair Gate and diff review only. Derive the detailed implementation plan from reviewed technical choices without prematurely converting Proposed to Accepted.

## Next Execution Plan

Current result: the single-device design and TDR-024/025 drafts are reviewable; hardware/new-APK direction is confirmed. Git status: versioned under bilingual governance; remote checks determine push status. Next action: review this design and both technology proposals, then detail the implementation plan in dependency order. Prerequisites: Owner review of scope and proposals; verify device API/connectivity and SDK readiness during implementation preflight. Acceptance target: clear test scope, identity, leases, simplified storage, failure semantics and real-device evidence boundaries without unresolved core-contract conflicts; not implementation or M3 acceptance.
