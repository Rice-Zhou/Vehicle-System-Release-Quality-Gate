...
Review version 2 confidence paths/local binding, Rule Set ownership of required Issues, the Case action table, asynchronous snapshot commit boundaries and parser/encoding preflight. An unselected usable parser version is an explicit implementation-plan preflight dependency, not completed feasibility testing.
