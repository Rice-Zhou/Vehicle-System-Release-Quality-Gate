...
- [x] **Step 6:** 独立工程复审、准确实施提交的双语 CI/Artifact/真机材料核对，验收记录使用新的固定实施 Subject，状态初始 PENDING；不能复用本设计 APPROVE。产品提交与记录提交分离，契约/验收校验、Pair Gate、原子推送与远端 HEAD 核对通过；填写实际已执行检查，不声称全部 M3 或 Company 完成。 最终工程复审与保留证据核对见[最终复审](../../m3/single-device-smoke-final-review.md)；固定产品汇总候选保持 PENDING，等待 Owner 决定。本勾选仅记录有界 Smoke 工程交付，不代表完整 M3 验收。
