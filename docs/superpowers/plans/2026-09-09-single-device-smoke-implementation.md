...
- [x] **Step 5:** 在实际支持范围内演练连接中断和 Agent 进程重启；不自动设备断电/重启。保存前置状态、注入方式、时序、旧/新租约、终态及恢复 bytes；确认没有重复安装、晚写或假 PASS。恢复 Server 数据与 Payload 副本，实际校验清单与结果；明示未覆盖完整 M3 Crash/ANR/断电出口。 实际支持的 HTTP 中断、Agent 重启与冷备副本恢复见[恢复验证](../../m3/smoke-recovery-verification.md)。保留原验证脚本失败，由独立补验核实同一 Run；物理 ADB 断连仍 UNKNOWN，Step 6 与 Owner 验收未关闭。
