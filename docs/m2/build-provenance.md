...
Gate 固定依次执行 contract、migration、canonical、validator、repository、transaction、security、github-smoke、contracts 和 acceptance。每项只输出 `CHECK <name> PASS|FAILED`、测试计数与固定 diagnostic，并保留首个真实失败 child exit code。缺少 GitHub context 时 `github-smoke` 必须以 `GITHUB_CONTEXT_MISSING` 失败；checkout HEAD 与 `GITHUB_SHA` 不同时以 `EXACT_HEAD_MISMATCH` 失败；新 Evidence 中的 `exactCommit`、`runId` 或 `runAttempt` 与当前 context 不同时以 `EVIDENCE_CONTEXT_MISMATCH` 失败。Evidence 必须严格符合封闭 schema：不允许缺字段、额外字段、敏感字段、错误类型、错误 digest/count、重复或缺少 typed Edge；违反时只输出固定 `EVIDENCE_INVALID`。本机缺少 Docker 时 PostgreSQL 检查以 `POSTGRESQL_RUNTIME_UNAVAILABLE` 失败。这些结果表示未执行，不得记录为 PASS，只能由绑定 exact commit 的 Linux/Docker GitHub Actions 补足。

成功的真实 Smoke 通过随机本地端口的实际 HTTP Endpoint 执行首次提交、same-key replay、different-key replay、Build Attempt conflict、USER 与 wrong-project 负向路径，再通过一个有界 JOIN projection 从 PostgreSQL 回读并绑定 Receipt、Snapshot、Commit、Build Attempt、三类当前 typed Revision、proof/fact digest、脱敏 Audit/Outbox 以及不存在 `ARTIFACT_RELEASE` 写入。Evidence 的 ID、digest 与 counts 只能从这组已验证值派生。它先写入最终文件同目录的唯一临时文件，完成序列化和 schema/context 校验后使用 atomic move 发布；任何失败都清理临时文件和最终文件，Gate 也拒绝残留临时文件。最终只生成 `backend/build/m2/build-provenance-smoke.json`，并由同一只读 workflow 上传为 `m2-build-provenance-${{ github.sha }}`。
