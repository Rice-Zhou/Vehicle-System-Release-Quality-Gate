...
This Profile is disabled by default. Explicitly configure `vsrqg.demo.evidence.enabled=true` and a pre-created absolute `vsrqg.demo.evidence.root` directory outside the repository, `static`, `public` and `wwwroot`. Startup and every file access check the root, ancestor links, regular files and service-account write permissions. POSIX rejects access permissions for other accounts/groups; Windows ACL trusts only the service account, SYSTEM and Administrators. This Profile sets Tomcat connection/upload idle timeouts to 30 seconds and disables swallowing the remaining request body after rejection. PUT additionally uses Servlet AsyncContext and a non-blocking ReadListener for a 30-second total receive deadline (`vsrqg.demo.evidence.upload-timeout` can only be configured within 1 ms–30 s). Timeout returns `408 UPLOAD_TIMEOUT` with requestId and cleans up the current temporary candidate. Read boundaries and EOF also check a monotonic clock, so continuous slow input cannot refresh the total deadline. These controls provide neither WORM nor tamper resistance against administrators.
...
Create strictly reuses Agent Schema and returns `{uploadId,evidenceId,uploadUrl,expiresAt}`. `uploadUrl` is a same-Backend relative URI that the Agent resolves against its configured mTLS Backend origin. Existing Create/Complete have no lease/fencing fields: Create captures both under the Attempt lock, and PUT/Complete compare the current binding each time. Lock order is Agent → Run → Attempt → Session. PUT receives network bytes after its short preflight transaction returns, holding no business locks while waiting for the body. After EOF, a short postflight transaction resolves current Agent permissions again, checks current Server time, Session/lease/terminal state, and compares the original binding. Only candidate size/hash matching the declaration permits create-only fixed-file publication and an UPLOADING commit. A short stream or incorrect hash deletes only the current candidate; existing correct files and correct orphans after DB rollback remain unchanged. Correct retransmission can use the same Session. EOF, timeout and error callback races have one terminal owner. Complete retains locks through Metadata, Audit and Outbox transaction commit.
