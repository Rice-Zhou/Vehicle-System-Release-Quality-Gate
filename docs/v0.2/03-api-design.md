...
Task 4 PUT reception uses Servlet asynchronous non-blocking reads: preflight and EOF postflight use separate short transactions, and network waits hold no Agent/Run/Attempt locks. The 30-second total deadline returns structured `408 UPLOAD_TIMEOUT`. Invalid size/hash candidates are rejected before fixed-file publication, allowing correct retransmission on the same Session. Postflight revalidates current permissions, lease, Session and terminal state using the certificate identity each time.
