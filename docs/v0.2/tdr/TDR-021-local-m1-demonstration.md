...
Local verifier unit tests need no Docker. Actual HTTP/database scenarios use PostgreSQL 17.11 and JDK 21, without substituting MockMvc jwt injection. Backend unit tests default to a 60-second timeout; add target tests to existing CI while preserving M1/M2 regression and bilingual Pair Gate. No available Docker command exists on this host currently; no demonstration was executed in this task.
...
Current result: Task 2 isolated launcher is complete; independent review, paired M1/M2 CI, and actual HTTP/database report verification passed. See the [implementation record](../../m1/2026-09-08-isolated-demo-launcher.md). Git state: paired implementation and record commits for this task. Next action: execute Task 3 single-command entry and result output. Prerequisite: next-step execution instruction; complete execution needs an existing container environment, without Company resources. Acceptance target: one command completes synthetic scenarios, emits sanitized results, exits nonzero on failure, and retains data; this does not substitute for Owner acceptance.
