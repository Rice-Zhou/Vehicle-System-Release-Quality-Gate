...
Current result: the Owner accepted the M1/M2 synthetic demonstrations and P0/P1 are closed; see the [updated gap inventory](2026-09-08-demonstrable-product-gap-inventory.md) for remaining scope. Git status: this inventory and navigation are versioned as bilingual documentation; remote verification determines push status. Next action: prepare a minimum TDR and implementation steps for the offline read-only M1/M2 demonstration report. Prerequisites: the next execution instruction; confirm scope before code implementation, without Company resources. Acceptance target: specify reused inputs, generation, bilingual delivery, failure semantics and verification methods, preserving business authority and original acceptance records.
