...
Current result: stage scope and Git preservation implemented; the [minimum gap inspection](2026-09-08-demonstrable-product-gap-inventory.md) is complete. Git state: these paired inspection-record commits. Next action: define the minimum synthetic M1 demonstration approach and implementation steps for startup/identity and actual sample-file checksum→Lock→export. Prerequisite: record technical choices and confirm scope before code implementation; no Company archive resources are needed. Acceptance target: cover both P0 inspection gaps while retaining authentication, database authority, and actual verification, without starting M3/M4.
