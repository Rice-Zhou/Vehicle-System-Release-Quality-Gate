...
Current result: closed M1/M2 demonstration gaps are distinguished from undelivered M3/M4/M5 capabilities, with one minimal M3 slice design package proposed. Git status: this record is versioned under bilingual governance; remote checks determine push status. Next action: design and plan the one-device, one-Smoke-Case Test Run→Result/Evidence flow. Prerequisites: a design execution instruction; confirm hardware and permitted operations during design, with resources and authorization in place before real execution. Acceptance target: fixed minimal scope, existing-contract mappings, necessary TDRs, failure verification and task order ready for implementation review; design acceptance does not constitute M3 acceptance.
