...
The scenario first checks the registered Agent. The formal Create Run API validates the project, Device, Locked Manifest and Published Plan. The Agent owns poll→Context→durable journal→ACK; the coordinator cannot query Context before dispatch. At completion, user APIs query Run→Result→Evidence, obtain separate download grants, bound download sizes, recompute each SHA-256, and read the same Run result again to confirm the historical projection remains stable. The summary excludes raw serials, account identities, JWTs, private keys, download grant URLs and local absolute paths.
