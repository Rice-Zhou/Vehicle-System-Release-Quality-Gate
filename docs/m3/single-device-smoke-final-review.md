...
当前结果：已将启动前提、正常/FAIL 顺序及证据定位补入[运行手册](single-device-smoke-runbook.md)，并对照既有脚本与报告结构完成只读交接核对；未产生新真机验证结论。Git 状态：本次文档单独版本化，提交由 Git history 定位。下一步动作：按手册完成一次历史证据展示。前置条件：下一步执行指令及受控输出可访问。验收目标：正常与确定 FAIL 的结果、Run/Attempt 关联及两类 Evidence 可定位，下载哈希与摘要一致，展示明确标注历史运行；缺失内容记录 UNKNOWN。
