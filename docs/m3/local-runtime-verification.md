...
Sole next action: use this controlled runtime to execute the accepted plan's normal and deterministic FAIL Smoke runs on the single head unit, then download LOG/PNG through formal APIs and recompute digests. Prerequisites: recheck the designated device connection, environment identity and authorized test-package operations before execution, and start this owned database; otherwise retain UNKNOWN. Acceptance target: distinct Runs with original Case PASS/FAIL respectively, consistent Run→Result→Evidence bindings and actual bytes/SHA-256, with normal completion after durable target ACK. Disconnection/restart and paired backup restoration remain separate checks; do not mark all Task 7 complete automatically.
