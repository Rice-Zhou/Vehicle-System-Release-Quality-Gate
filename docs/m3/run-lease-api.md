...
Heartbeat, Worker and AttemptAccess share current execution eligibility checks: registration remains valid, capabilities satisfy the fixed Case, and vehicle/platform still match the Locked Manifest. Loss of eligibility prevents renewal and write access. Heartbeat/Worker atomically record ERROR and release the reservation; AttemptAccess preserves rejection within the caller transaction. Each Worker tick traverses active Runs by Run ID cursor in pages of at most 100, without reading terminal history, preventing continuing renewals from hiding eligibility changes or starving later pages. Recovery renewal uses the pre-recovery ACK state. Recovering an unacknowledged DISPATCHED Attempt only clears recovery markers and leaves lease expiry unchanged.
