...
Sole next action: execute plan Task 7 Step 5 controlled connection interruption, Agent process restart and paired database+Payload restoration. Prerequisites: explicit injection scope, device/runtime authorization and controlled backup copies; no automatic device power loss/reboot. Acceptance target: retained before/after timing, leases/terminal states and restored bytes/SHA-256 proving no duplicate installation, late writes or false PASS. Unexecuted checks remain UNKNOWN. This next step does not automatically close Task 7 or replace an Owner decision.
